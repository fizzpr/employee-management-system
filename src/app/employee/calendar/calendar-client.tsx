...
export default function CalendarClient({ attendances, completedTasks }: CalendarClientProps) {
  ...
  const getCompletedTasksForDay = (day: number) => {
    const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
    return completedTasks.filter((t) => t.completedAt.startsWith(dateStr));
  };
        ...
        <div className="space-y-1.5">
          ...
          {statusText && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-[10px] font-bold text-slate-500 capitalize">{statusText}</span>
            </div>
          )}
  ...
  return (
    <div className="space-y-8 p-8">
      ...
      <div>
        ...
        <p className="text-xs text-slate-500">Track and review attendance history date-wise</p>
      </div>
                ...
                <div className="space-y-4">
                  {selectedDayInfo.attendance ? (
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          <span
                            className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${
                              ...
                              selectedDayInfo.attendance.status === 'LATE' ? 'bg-amber-50 text-amber-700 border border-amber-100' :
                    ...
                    <div className="text-center rounded-xl bg-red-50 p-4 border border-red-100">
                      ...
                      <p className="text-[10px] text-red-500 mt-1">No attendance logs found for this date.</p>
                    </div>
                  )}
                </div>
